...
from real_estate_telegram_bot.core.utils import create_keyfile_dict
    ...
    def get_folder_id(self, folder_name: str) -> Optional[str]:
        ...
        try:
            ...
            if folder_list:
                ...
                folder_list = self.drive.ListFile(
                    {
                        "q": f"title = '{folder_name.lower()}' and mimeType = 'application/vnd.google-apps.folder' and trashed = false"
                    }
                ).GetList()
                if folder_list:
                    return folder_list[0]["id"]
